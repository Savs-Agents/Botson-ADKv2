...
// /.well-known/agent-card.json) is untouched, and Server.Run (server.go)
// mounts this handler at those same paths.
func NewReverseProxy(backendBaseURL string, requestTimeout time.Duration, logger *slog.Logger) (http.Handler, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("api: parse backend URL %q: %w", backendBaseURL, err)
	}
	if logger == nil {
		logger = slog.Default()
	}
	...
	// whole agent turn (see adkbackend.go's serverWriteTimeout doc comment
	// on why real turns run for minutes, not seconds). Callers should pass
	// a value with real headroom above their longest expected turn.
	proxy.Transport = &http.Transport{ResponseHeaderTimeout: requestTimeout}
	proxy.ErrorHandler = func(w http.ResponseWriter, r *http.Request, err error) {
		logger.Error("api: backend request failed", "path", r.URL.Path, "error", err)
